...
class MySimulator(QObject, PyCustomerSimulator, SimuInterface, IVehicleDrivingManagerTask):
    ...
    def __init__(self):
        ...
        # 记录本次仿真确定的安全时距、距离
        self.mAutoInOutSi = 1.5
        self.mAutoInOutSd = 1.5
    ...
    def ref_reSetFollowingParam(self, vehi, ref_inOutSi, ref_inOutSd):
        ref_inOutSi.value = m2p(self.mAutoInOutSi)
        ref_inOutSd.value = m2p(self.mAutoInOutSd)
        return True
    ...
    def afterOneStep(self):
        ...
        if batchNum % 20 == 0:
            ...
            strInOutSi = str(self.mAutoInOutSi)
            strInOutSd = str(self.mAutoInOutSd)
        ...
        # 当仿真时间为10秒时停止仿真
        if batchNum == 200:
            #iface.IVehicleDrivingManagerTask.setStopped()
            #调用停止仿真方法会引发tess崩溃
            #simuiface.stopSimu()
            pass
