...
class MySimulator(QObject, PyCustomerSimulator):
    ...
    def calcLimitedLaneNumber(self, pIVehicle):
        ...
        count = 0  # pIVehicle：在ID=3的路段上的车辆
        GL_list = [0, 1, 2]  # GL车道编号
        ML_list = [3]  # ML车道编号
        # 行驶距离大于 1049 小于 1433
        dist = p2m(pIVehicle.vehicleDriving().getVehiDrivDistance())  # 车辆已经行驶距离（单位：像素 -> 转为米）
        if (pIVehicle.roadId() == 3 or pIVehicle.roadId() == 7) and 1427 >= dist >= 1146:   #1046
            ...
            if pIVehicle.vehicleTypeCode() == 13:
                ...
                if batchNum % (20 * 60) == 0:
                    if self.ML2GL:
                        return ML_list
                    elif self.GL2ML and pIVehicle.vehicleDriving().laneNumber() == 2:
                        return GL_list
    ...
    def afterOneStep(self):
        ...
        if len(lVehisInfoAggr) > 0:
            ...
            self.Q_GL = self.Q6 + self.Q7 + self.Q8
        ...
        if self.V_GL != 0 and self.MLFlow != 0 and self.GLFlow != 0:
            ...
            if (self.V_ML - self.V_GL) / self.V_GL > speedDiffRate:
                ML_overload = 'no'
                # print("ML is not overloaded, can run more GVs.")
                ...
                while(self.MLFlow<=3600 and self.GLFlow>=0):
                    ...
                    if self.tempSpeedDiffRate <= speedDiffRate:  # 如果速度差满足了标准 那么就给出结果 并结束循环
                        ...
                        # 普通车道变到管控车道信号为True
                        self.GL2ML = 1
                       # print("需要从普通车道变道至管控车道")
                        # 把车移到管控车道
                       # print("管控车道方案流量", self.solutionMLFlow, self.Q_ML)
