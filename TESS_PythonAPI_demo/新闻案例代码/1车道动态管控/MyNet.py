...
class MyNet(PyCustomerNet):
    ...
    def createNet(self):
        ...
        lPoint = [startPoint, endPoint]
        link3 = netiface.createLink(lPoint, 3, "路段3")
        ...
        lPoint = [startPoint, endPoint]
        link4 = netiface.createLink(lPoint, 3, "路段4")
        ...
        if link3 is not None and link4 is not None:
            ...
            lToLaneNumber = [1, 2, 3]
            conn1 = netiface.createConnector(
                link3.id(), link4.id(), lFromLaneNumber, lToLaneNumber, "连接段1", True)
        ...
        if link7 is not None and link8 is not None:
            lFromLaneNumber = [1, 2, 3]
            lToLaneNumber = [1, 2, 3]
            conn3 = netiface.createConnector(link7.id(), link8.id(), lFromLaneNumber, lToLaneNumber, "动态发车连接段", True)
